...
wlda_acc <- augment(wlda_fit, new_data = white_test) %>% accuracy(truth = quality, estimate = .pred_class) %>% mutate(model_type = "White LDA Model")
...
rlda_acc <- augment(rlda_fit, new_data = red_test) %>% accuracy(truth = quality, estimate = .pred_class)%>% mutate(model_type = "Red LDA Model")
