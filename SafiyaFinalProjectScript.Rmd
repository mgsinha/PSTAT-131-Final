...
collect_metrics(wtune_res_randfor) %>% arrange(desc(mean))
best_metric1 <- select_best(tune_res_poke_randfor)

randfor_poke_final <- rand_forest(mtry = 2, trees = 17, min_n = 32) %>% set_engine("ranger", importance = "impurity") %>% set_mode("classification")

randfor_fit_final <- fit(randfor_poke_final, type_1 ~ hp + attack + defense + sp_atk + sp_def + speed + generation + legendary, data = pokemon_training)
