...
quality_df <- table(combinedWine$Quality)

#rename column to Quality
names(quality_df)[names(quality_df) == "Var1"] <- "Quality"

barplot(quality_df, main = 'Frequency of Quality values', xlab = "Value of Quality", ylab = "Frequency") 
#quality_plot <- ggplot(data.frame(quality_df), aes(x = Quality, y = Freq, fill = Quality)) + geom_bar(stat = "identity") + ggtitle("Frequency of Quality values") + xlab("Value of Quality") + ylab("Frequency")
#quality_plot
```

```{r separated quality histogram}
ggplot(train, aes(difficulty)) +
