...
We will be fitting linear discriminant analysis, naive Bayes, single decision tree, random forest, and boosted tree models and compare accuracy metrics. Then, we will fit the best model to our testing data. First, let's see how the models perform on the white wine data set.
...
Through k-fold cross validation, we can see that the linear discriminant analysis model produces a more accurate model than the Naive Bayes model. Thus, we will use linear discriminant analysis on the white wine data set. Now, let's explore how the model performs.
...
augment(wclass_tree_final_fit, new_data = white_train) %>%
  accuracy(truth = quality, estimate = .pred_class)
...
augment(wclass_tree_final_fit, new_data = white_test) %>%
  conf_mat(truth = quality, estimate = .pred_class) 
