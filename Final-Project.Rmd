...
We only get a slightly better accuracy metric using principle component analysis on a linear discriminant model. 
...
Now we produce a graph of ____

```{r white decision tree autoplot code, eval=FALSE, class.source = 'fold-show'}
...
```{r white random forest tuning, class.source = 'fold-show', eval=FALSE}
...
```{r white random forest best fit, eval = FALSE}
...
```{r white random forest variance importance plot, eval = FALSE}
...
``` {r white boosted tree grid, eval=FALSE, class.source = 'fold-show'}
...
``` {r white boosted tree final fit visual}
wboost_final <- boost_tree(tree_depth = 5, trees = 231)%>% 
  set_engine("xgboost") %>% 
  set_mode("classification")
```

```{r white boosted tree final fit, eval=FALSE, class.source = 'fold-show'}
...
```{r white boosted tree augment, eval=FALSE}
# augmenting 
...
```{r white PCA LDA testing code, eval=FALSE}
...
```{r white random forest testing code, eval=FALSE}
...
```{r white boosted trees testing code, eval=FALSE}
...
```{r red pca lda recipe, eval=FALSE, class.source = 'fold-show'}
...
``` {r red pca lda tuning, eval=FALSE, class.source = 'fold-show'}
# column name(s) must match tune() above
tuneGrid <- expand.grid(num_comp = 1:ncol(red_recipe_pca$template))
```

```{r red pca lda tune grid, eval=FALSE, class.source = 'fold-show'}
# control tune_grid() process below
trControl <- control_grid(verbose = TRUE, allow_par = FALSE)
```

```{r red pca lda workflow and fit, eval=FALSE, class.source = 'fold-show'}
...
```{r red pca lda tibble, eval=FALSE}
...
```{r red pca lda final fit, eval=FALSE}
...
```{r red decision tree fit visual, eval=FALSE, class.source = 'fold-show'}
rclass_tree_fit %>%
  extract_fit_engine() %>%
  rpart.plot()
```

```{r red decision tree augment training, eval=FALSE, class.source = 'fold-show'}
# augmented on training 
...
```{r red decision tree augment testing, eval=FALSE, class.source = 'fold-show'}
...
``` {r red decision tree tuning grid, eval=FALSE}
param_grid <- grid_regular(cost_complexity(range = c(-3,-1)), levels = 10)
...
```{r red decision tree best complexity, eval=FALSE}
# extracting the best cost complexity parameter
...
```{r red decision tree augment training final, eval=FALSE}
# augmented on training 
...
```{r red random forest set up, message=FALSE, eval=FALSE}
...
```{r red random forest tuning, eval=FALSE, class.source = 'fold-show'}
# tuning the model to find the best arguments 
...
```{r red random forest vip, eval=FALSE, class.source = 'fold-show'}
...
```{r red boosted tree spec, message=FALSE, eval=FALSE}
...
```{r red boosted tree tuning grid, eval=FALSE, class.source = 'fold-show'}
...
```{r PCA LDA testing code, eval=FALSE}
...
```{r random forest testing code, eval = FALSE}
...
```{r boosted tree testing code, eval = FALSE}
